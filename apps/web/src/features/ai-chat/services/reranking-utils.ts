import { createRerankingPrompt } from '../domain/prompts';
...
export function selectTopDocumentsWithThreshold(
  ...
  const aboveThreshold = sorted.filter(
    (doc) => doc.relevanceScore >= minThreshold,
  );
  ...
  if (aboveThreshold.length >= targetCount) {
    ...
    const allAtCutoff = aboveThreshold.filter(
      (doc) => doc.relevanceScore >= cutoffScore,
    );
  ...
  if (aboveThreshold.length > 0) {
    ...
    const belowThreshold = sorted.slice(
      aboveThreshold.length,
      aboveThreshold.length + needed,
    );
  ...
  const effectiveThreshold =
    selected.length > 0 ? selected[selected.length - 1].relevanceScore : 0;
