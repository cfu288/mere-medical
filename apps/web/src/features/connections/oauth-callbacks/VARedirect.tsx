...
import { saveConnectionToDb } from '../../../services/fhir/VA';

const vaClient = createVAClient();

function useVAOAuthCallback() {
  const db = useRxDb();
  ...
  const { clearSession } = useOAuthorizationRequestState('va');
  ...
  useEffect(() => {
    ...
    if (!config.VA_CLIENT_ID || !config.PUBLIC_URL) {
      hasRun.current = true;
      clearSession();
      const errorMessage = 'VA OAuth configuration is incomplete';
    ...
    if (!user?.id) {
      hasRun.current = true;
      clearSession();
      const errorMessage = 'User session not found';
      setError(errorMessage);
      notifyDispatch({
        type: 'set_notification',
        message: errorMessage,
...
const VARedirect: React.FC = () => {
  const navigate = useNavigate();
  const error = useVAOAuthCallback();
