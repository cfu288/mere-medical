...
function clearLocalStorage() {
  Object.values(AthenaLocalStorageKeys).forEach((key) =>
    localStorage.removeItem(key),
  );
}

function useAthenaOAuthCallback() {
  ...
  useEffect(() => {
    ...
    const athenaEnvironment = localStorage.getItem(
      AthenaLocalStorageKeys.ATHENA_ENVIRONMENT,
    ) as 'preview' | 'production' | null;
    ...
    if (!clientId || !config.PUBLIC_URL) {
      ...
      notifyDispatch({
        type: 'set_notification',
        message: 'Athena OAuth configuration is incomplete',
        variant: 'error',
      });
      navigate(Routes.AddConnection);
      return;
...
const AthenaRedirect: React.FC = () => {
  useAthenaOAuthCallback();
