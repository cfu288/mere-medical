...
describe('resolveSyncContext', () => {
  it('resolves an Epic base url from the published tenant', () => {
  ...
  it('keeps a legacy Epic full fhir path when the tenant is unpublished', () => {
    const result = resolve({
      ...epicBase,
  ...
  it('fails when an unpublished Epic tenant stores only an origin', () => {
    const result = resolve({
      ...epicBase,
