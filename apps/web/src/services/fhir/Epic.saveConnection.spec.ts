...
describe('saveConnectionToDb', () => {
  ...
  const connect = (epicId: string, epicName: string, patient: string) =>
    saveConnectionToDb({
      res: {
        access_token: `token-for-${patient}`,
        expires_in: 3600,
        patient,
        token_type: 'Bearer',
        scope: 'patient/*.read',
  ...
  it('updates in place when the same tenant reconnects', async () => {
    await connect(UCSF_HEALTH_ID, 'UCSF Health', 'patient-health');
    await connect(UCSF_HEALTH_ID, 'UCSF Health', 'patient-health-renewed');

    const connections = await connectionRepo.findAllConnections(db, user.id);

    expect(connections).toHaveLength(1);
    expect(connections[0].patient).toBe('patient-health-renewed');
  });
});
