...
OnPatient requires an HTTPS redirect URL, so Mere needs a real domain name behind a reverse proxy — see [running behind a reverse proxy](./docker#running-behind-a-reverse-proxy).
