...
import { openWarehouse, readMigrations } from './open';
...
describe('schema drift', () => {
  ...
  it('numbers migrations from one with no gaps', () => {
    expect(readMigrations().map((migration) => migration.version)).toEqual([1]);
  });

  it('leaves user_version at the highest applied migration', () => {
    expect(db.prepare('PRAGMA user_version').get()).toEqual({
      user_version: readMigrations().length,
    });
  });
